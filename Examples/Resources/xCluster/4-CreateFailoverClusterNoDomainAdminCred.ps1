...
Configuration Example
{
    ...
    Node $AllNodes.Where{$_.Role -eq 'FirstServerNode' }.NodeName
    {
        ...
        # NOTE: You must ACL your Active Directory OU/Container correctly for the system identity to be able to create the CNO.
    ...
    Node $AllNodes.Where{ $_.Role -eq 'AdditionalServerNode' }.NodeName
    {
        ...
        # NOTE: Your server must have local administrator rights on all other members of the cluster (so it can run validation tests / WMI&RPC connect.)
